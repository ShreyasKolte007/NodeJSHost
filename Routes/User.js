...
route.get("/shop",async function(req,res){
    ...
    var company_name = await exe(`SELECT product_company FROM product GROUP BY product_company`);
    ...
    WHERE product.product_id = product_pricing.product_id
    AND product_duplicate_price > 0) AS product_duplicate_price FROM product`+ cond;
    ...
    // SELECT MIN(product_price) FROM product_pricing;
    // SELECT MAX(product_duplicate_price) FROM product_pricing;
    var obj = {"about_company":data,"category":category,"company_name":company_name,"color":color,"product_info":product_info,"is_login":verifyaccount(req)};
...
route.post("/create_account",async function(req,res){
    var d = req.body;
    var sql = `INSERT INTO users(user_name,user_mobile,useremail,password) VALUES
    (?,?,?,?)`
    var data = await exe(sql,[d.user_name,d.user_mobile,d.useremail,d.password])
    res.redirect('/login')
})
...
route.get("/add_to_cart/:product_id/:product_pricing_id",async function(req,res){
    ...
    var user_id = req.session.user_id;
    // Check if user is logged in before attempting to insert into cart
    if(req.session.user_id){
        var sql = `INSERT INTO cart (product_id,product_pricing_id,user_id,qty) VALUES ('${product_id}','${product_pricing_id}','${user_id}',1)`;
        var data = await exe(sql);
        res.redirect("/shop")
    }else{
        res.redirect("/login")
...
route.get("/cart_remove/:cart_id",async function(req,res){
    var cart_id = req.params.cart_id;
...
route.post("/order",async function(req,res){
    ...
    var sql1 = `INSERT INTO order_table (customer_name,customer_mobile,customer_state,customer_district,customer_city,customer_area,customer_landmark,
    customer_pincode,payment_type,order_date,order_amount,payment_status,order_status) VALUES 
    ('${d.customer_name}',
     '${d.customer_mobile}',
     '${d.state}',
     '${d.district}',
     ...
     for(var i=0;i<cart.length;i++){
        var sql2 = `INSERT INTO order_detail(order_id,product_id,customer_id,product_pricing_id,product_name,product_price
        ,product_color,product_size,product_image1,product_company,product_qty,product_total) VALUES ('${order_id}',
        '${cart[i].product_id}',
        '${req.session.user_id}',
...
route.get("/order_info/:order_id",async function(req,res){
    ...
    var data = await exe(`SELECT * FROM company_details`);
    var obj = {about_company:data,"is_login":verifyaccount(req),"order_data":order_data,"order_detail":order_detail};
    res.render("user/order_info.ejs",obj);
})

route.get("/payment/:order_id",async function(req,res){
    var order_id = req.params.order_id;
    var data = await exe(`SELECT * FROM order_table WHERE order_id = '${order_id}'`);
    console.log(data);
    var obj = {"data":data}
...
route.post("/check_payment/:order_id",async function(req,res){
    var order_id = req.params.order_id;
    var sql = `UPDATE order_table SET transaction_id = '${req.body.razorpay_payment_id}' WHERE order_id = '${req.params.order_id}'`;
    var data = await exe(sql);
    res.redirect('/order_info/'+order_id);
})
